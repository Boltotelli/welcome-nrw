...
function neutralizeMocks(){
 const concept=document.querySelector('.concept');if(concept)concept.textContent='NAP Event Tracker 2.0';
}
...
async function renderPerformanceLive(){
 ...
 try{
  ...
  v.innerHTML='<div class="hero"><div><div class="kicker">PERFORMANCE · LIVE</div><h1>Performance ohne Dashboard-Überladung.</h1><p>'+E(S.a)+' · höchste Werte pro Spieler</p></div><div class="hero-actions"><button class="btn primary" id="livePerfScreen">ScreenRecording importieren</button><button class="btn secondary" id="livePerfManual">Manuell ergänzen / korrigieren</button></div></div>'+
   ...
   '<div class="performance-kpi-grid"><div class="performance-kpi compact"><b>'+N(m?.recorded_players||0)+'</b><small>erfasste Spieler</small></div><div class="performance-kpi score"><b>'+N(m?.total_score||0)+'</b><small>Gesamtscore</small></div><div class="performance-kpi score"><b>'+N(m?.average_score||0)+'</b><small>Ø Score</small></div><div class="performance-kpi compact"><b>'+N(m?.roster_players||0)+'</b><small>Roster</small></div></div>'+perfTopList2(m?.top5,false)+'<div class="hero-actions" style="margin-top:10px"><button class="btn tertiary live-full-ranking" data-type="mob">Vollständiges Ranking</button></div></div></section>'+
   ...
   '<div class="performance-kpi-grid"><div class="performance-kpi compact"><b>'+N(k?.known_top200_players||0)+'</b><small>Top-200 Spieler</small></div><div class="performance-kpi score"><b>'+N(k?.known_top200_score||0)+'</b><small>bekannter Score</small></div><div class="performance-kpi score"><b>'+N(k?.alliance_prep_score||0)+'</b><small>Alliance Prep</small></div><div class="performance-kpi compact"><b>'+E(S.a)+'</b><small>Allianz</small></div></div>'+perfTopList2(k?.top5,true)+'<div class="hero-actions" style="margin-top:10px"><button class="btn tertiary live-full-ranking" data-type="kvk">Vollständiges Ranking</button></div></div></section></div><div id="livePerformanceExtra" style="margin-top:14px"></div>';
...
function law9Normalized2(v){
 ...
 return Number(v).toLocaleString(L()==='de'?'de-DE':L()==='fr'?'fr-FR':L()==='es'?'es-ES':'en-GB',{minimumFractionDigits:0,maximumFractionDigits:6});
}
async function loadLaw9Display2(){
 let law=await rpc('get_law9_dashboard',law9SelectedCycle2?{p_cycle_id:law9SelectedCycle2}:{});
 if(!law9SelectedCycle2&&Number(law?.cycle?.score_count||0)===0){
  const previous=(law?.cycles||[]).find(x=>Number(x?.score_count||0)>0);
  if(previous?.id){
   law9SelectedCycle2=previous.id;
   law=await rpc('get_law9_dashboard',{p_cycle_id:previous.id});
  }
 }
 return law;
}
async function renderKvkLive(){
 ...
 try{
  const [law,perf]=await Promise.all([loadLaw9Display2(),rpc('get_performance_dashboard',{})]);S.law9=law;S.performance=perf;
  const cycle=law?.cycle||{},ranking=law?.ranking||[],base=law?.baselines||[],memberPlan=law?.member_plan||[],canEditMembers=!!law?.can_edit_member_counts,cycles=law?.cycles||[];
  law9SelectedCycle2=cycle.id||law9SelectedCycle2;
  v.innerHTML='<div class="hero"><div><div class="kicker">KVK · LIVE</div><h1>KvK & Law 9</h1><p>'+E(D(cycle.prep_start))+' · '+E(law?.formula||'')+'</p></div><div class="hero-actions"><label style="display:flex;align-items:center;gap:8px;color:var(--muted);font-size:12px">KvK-Zyklus<select id="liveLaw9Cycle" class="compact-select">'+cycles.map(x=>'<option value="'+E(x.id)+'" '+(String(x.id)===String(cycle.id)?'selected':'')+'>'+E(law9CycleLabel2(x))+' · '+N(x.score_count||0)+'/6</option>').join('')+'</select></label></div></div>'+
   ...
   '<div class="live-panel-grid"><section class="card"><div class="card-head"><div><div class="card-title">🏆 Sanctuary Selection Ranking</div><div class="card-sub">Law #9 · Prep Score / ((Alliance Power^0.7) × (Members^0.3))</div></div><span class="pill">'+N(cycle.score_count||0)+'/6</span></div><div class="card-body live-list">'+
   (ranking.length?ranking.map(r=>'<div class="live-row"><div><b><span style="display:inline-flex;align-items:center;gap:8px">'+(r.rank?'#'+r.rank+' · ':'')+allianceLogo2(r.alliance_code,'alliance-logo-img')+'<span>'+E(r.alliance_code)+'</span></span></b><small>Power '+N(r.alliance_power)+' · '+N(r.member_count)+' Mitglieder</small></div><div style="text-align:right"><strong>'+N(r.prep_score||0)+'</strong><small style="color:var(--blue-text,var(--blue));font-weight:850">'+law9Normalized2(r.normalized_score)+'</small></div></div>').join(''):'<div class="live-empty-state">Noch kein Ranking für diesen Zyklus.</div>')+
...
const ACTIVITY_EXTRA_TITLES2={
 de:{
  ...
  screen_import_occurrence_created_test:'ScreenRecording erfasst',screen_import_phase_corrected:'ScreenRecording Phase korrigiert',
 ...
 en:{
  ...
  screen_import_occurrence_created_test:'ScreenRecording recorded',screen_import_phase_corrected:'ScreenRecording phase corrected',
 ...
 fr:{
  ...
  screen_import_occurrence_created_test:'ScreenRecording enregistré',screen_import_phase_corrected:'Phase ScreenRecording corrigée',
