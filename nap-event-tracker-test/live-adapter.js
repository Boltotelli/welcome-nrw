...
async function renderActivityLive(){
 ...
 try{const d=await rpc('get_activity_log',{p_limit:250,p_offset:0,p_alliance:S.a,p_action:null});S.activity=Array.isArray(d)?d:[];paintActivity2()}catch(err){v.innerHTML+='<div class="live-empty-state">'+E(err.message||String(err))+'</div>'}
}
function paintActivity2(){
 ...
 '<div class="toolbar"><div class="toolbar-left">'+types.map(x=>'<button class="filter-chip '+(activityType2===x[0]?'active':'')+'" data-acttype="'+x[0]+'">'+x[1]+'</button>').join('')+'</div><div class="toolbar-right"><span class="pill">'+E(S.a)+'</span></div></div>'+
 ...
 v.querySelectorAll('[data-acttype]').forEach(b=>b.onclick=()=>{activityType2=b.dataset.acttype;paintActivity2()});document.getElementById('liveActivityRefresh').onclick=renderActivityLive;
}
