...
function renderPlayers2(){
 ...
 }).join('')+(S.a==='NRW'?'<div class="player-card n2-demo-player" data-has-entry="1" data-attendance="0" data-search="[demo] nrw shared spending" role="button" tabindex="0" style="cursor:pointer;border-style:dashed"><div class="player-card-top"><div class="player-meta"><div><div class="player-name">🧪 '+E(sharedDemoWords2().player)+'</div><div class="player-id">'+E(sharedDemoWords2().tag)+'</div></div></div>'+allianceBadge2('NRW')+'</div><div class="metric-row"><div class="metric"><b>2</b><span>Demo-Events</span></div><div class="metric"><b>0</b><span>echte Sanktionen</span></div></div><div class="player-card-foot"><span class="pill gold">'+E(sharedDemoWords2().cta)+'</span></div></div>':'')||'<div class="live-empty-state">Keine Spieler.</div>';
 g.querySelectorAll('.n2-demo-player').forEach(card=>{card.onclick=openSharedSpendingDemo2;card.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();openSharedSpendingDemo2()}}});
...
/* NRW-only, browser-only Shared Spending sample. No fake players, violations or sanctions
   are added to S.* or Supabase; the decision is never sent to any RPC. */
const SHARED_DEMO_WORDS2={
 de:{title:'[DEMO] NRW Shared Spending',cta:'Beispieldatensatz testen',tag:'NUR NRW · SIMULATION',intro:'Fiktiver Spieler, fiktiver gemeinsamer Eventtag. Kein Supabase-Datensatz, kein echter Verstoß und keine echte Sanktion.',player:'[DEMO] NRW Shared Spending',scenario:'Ein und dieselben Charms geben Punkte in zwei Events.',choose:'Entscheidung ausprobieren',pending:'Standard · keine Ausnahme',shared:'Ausnahme · gemeinsame Ausgaben',separate:'Getrennte Ausgaben',reason:'Begründung (nur bei gemeinsamer Ausgabe erforderlich)',save:'Testentscheidung simulieren',reset:'Demo zurücksetzen',end:'Demo schließen',impact:'Simuliertes Ergebnis',baseline:'Ohne bestätigte Ausnahme werden beide Events einzeln gezählt.',one:'Beide Eventverstöße bleiben bestehen, zählen aber zusammen als ein Sanktionsfall.',two:'Beide Eventverstöße zählen einzeln. Keine Ausnahme.',stages:'Beispiel ohne frühere Verstöße oder Kontaktaufnahme:',details:'Die echten Verstöße, Screenshots, Sanktionsstufen und Spielerakten werden nicht verändert.',invalid:'Bitte die gemeinsamen Ressourcen begründen (mindestens 8 Zeichen).',saved:'Nur in dieser Vorschau simuliert. Es wurde nichts gespeichert.',day:'Fiktiver Eventtag',goal:'Ziel',threshold:'3× Grenze',score:'Score'},
 en:{title:'[DEMO] NRW Shared Spending',cta:'Try sample data',tag:'NRW ONLY · SIMULATION',intro:'Fictional player and event day. No database record, actual violation or sanction.',player:'[DEMO] NRW Shared Spending',scenario:'The same Charm spending scores in two events.',choose:'Try a decision',pending:'Default · no exception',shared:'Exception · shared spending',separate:'Separate spending',reason:'Reason (required only for shared spending)',save:'Simulate decision',reset:'Reset demo',end:'Close demo',impact:'Simulated result',baseline:'Without an approved exception, both events count separately.',one:'Both event violations remain, but count together as one sanction case.',two:'Both violations count separately. No exception.',stages:'Example with no earlier violations or contact:',details:'Real violations, evidence, sanctions and player files are unchanged.',invalid:'Explain the shared resources (at least 8 characters).',saved:'Simulated only in this preview. Nothing was saved.',day:'Fictional event day',goal:'Goal',threshold:'3× limit',score:'Score'},
 fr:{title:'[DEMO] NRW Shared Spending',cta:'Essayer le jeu d’essai',tag:'NRW UNIQUEMENT · SIMULATION',intro:'Joueur et journée fictifs. Aucun enregistrement, aucune infraction ni sanction réelle.',player:'[DEMO] NRW Shared Spending',scenario:'Les mêmes charmes donnent des points dans deux événements.',choose:'Tester une décision',pending:'Par défaut · aucune exception',shared:'Exception · dépenses communes',separate:'Dépenses distinctes',reason:'Motif (obligatoire uniquement pour les dépenses communes)',save:'Simuler',reset:'Réinitialiser',end:'Fermer',impact:'Résultat simulé',baseline:'Sans exception confirmée, les deux événements comptent séparément.',one:'Les deux infractions restent visibles mais comptent comme un seul cas.',two:'Les deux infractions comptent séparément.',stages:'Exemple sans infraction ni contact antérieur :',details:'Les véritables infractions, preuves, sanctions et dossiers restent inchangés.',invalid:'Expliquez les ressources communes (8 caractères minimum).',saved:'Simulation uniquement. Rien n’a été enregistré.',day:'Jour fictif',goal:'Objectif',threshold:'Limite 3×',score:'Score'},
 es:{title:'[DEMO] NRW Shared Spending',cta:'Probar datos de ejemplo',tag:'SOLO NRW · SIMULACIÓN',intro:'Jugador y día ficticios. Sin registros, infracciones ni sanciones reales.',player:'[DEMO] NRW Shared Spending',scenario:'Los mismos amuletos otorgan puntos en dos eventos.',choose:'Probar una decisión',pending:'Predeterminado · sin excepción',shared:'Excepción · gastos compartidos',separate:'Gastos separados',reason:'Motivo (obligatorio solo para gastos compartidos)',save:'Simular decisión',reset:'Reiniciar',end:'Cerrar',impact:'Resultado simulado',baseline:'Sin excepción aprobada, ambos eventos cuentan por separado.',one:'Ambas infracciones permanecen, pero cuentan como un solo caso.',two:'Ambas infracciones cuentan por separado.',stages:'Ejemplo sin infracciones ni contacto previos:',details:'No se modifican las infracciones, pruebas, sanciones ni expedientes reales.',invalid:'Explica los recursos compartidos (mínimo 8 caracteres).',saved:'Solo se ha simulado; no se guardó nada.',day:'Día ficticio',goal:'Objetivo',threshold:'Límite 3×',score:'Puntos'}
};
function sharedDemoWords2(){return SHARED_DEMO_WORDS2[L()]||SHARED_DEMO_WORDS2.de}
function openSharedSpendingDemo2(){
 if(S.a!=='NRW')return;
 document.getElementById('n2SharedSpendingDemo')?.remove();
 const w=sharedDemoWords2(),modal=document.createElement('div');
 modal.id='n2SharedSpendingDemo';modal.className='modal-backdrop';
 const rows=[
   {event:'Strongest Governor',phase:'Day 5',target:296000,score:910000},
   {event:'Officer Project',phase:'Charms',target:310000,score:960000}
 ];
 let result='pending';
 const frame=()=>'<div class="modal-card" role="dialog" aria-modal="true" aria-label="'+E(w.title)+'" style="width:min(760px,95vw);max-height:90dvh;overflow:auto">'+
 '<div class="modal-head"><div><b>'+E(w.title)+'</b><small>'+E(w.tag)+'</small></div><button type="button" class="icon-btn n2-demo-close" aria-label="'+E(w.end)+'">×</button></div>'+
 '<div class="live-note" style="margin:10px 0"><b>'+E(w.intro)+'</b></div>'+
 '<p class="muted small">'+E(w.day)+': 17.09.2026 · '+E(w.scenario)+'</p>'+
 '<div class="live-list">'+rows.map(r=>'<div class="live-row"><div><b>'+E(r.event)+' · '+E(r.phase)+'</b><small>'+E(w.goal)+' '+N(r.target)+' · '+E(w.threshold)+' '+N(r.target*3)+'</small></div><span class="pill red">'+E(w.score)+' '+N(r.score)+'</span></div>').join('')+'</div>'+
 '<form class="live-form n2-shared-demo-form" style="margin-top:14px"><label>'+E(w.choose)+'<select class="n2-shared-demo-status"><option value="pending" '+(result==='pending'?'selected':'')+'>'+E(w.pending)+'</option><option value="shared" '+(result==='shared'?'selected':'')+'>'+E(w.shared)+'</option><option value="separate" '+(result==='separate'?'selected':'')+'>'+E(w.separate)+'</option></select></label>'+
 '<label>'+E(w.reason)+'<textarea class="n2-shared-demo-reason" rows="2" maxlength="500" placeholder="'+E(w.scenario)+'"></textarea></label>'+
 '<div class="hero-actions"><button type="submit" class="btn primary">'+E(w.save)+'</button><button class="btn secondary n2-demo-reset" type="button">'+E(w.reset)+'</button></div><div class="live-status n2-demo-status"></div></form>'+
 '<section class="card" style="margin-top:14px"><div class="card-head"><div><div class="card-title">'+E(w.impact)+'</div><div class="card-sub">'+E(w.stages)+'</div></div></div><div class="card-body"><div class="metric-row"><div class="metric"><b>2</b><span>'+E(sharedWords2().title).replace(/\s*·.*/,'')+' · Events</span></div><div class="metric"><b>'+(result==='shared'?'1':'2')+'</b><span>Law 14 · '+(result==='shared'?'Stufe 1':'Stufe 2')+'</span></div></div><div class="live-note">'+E(result==='shared'?w.one:result==='separate'?w.two:w.baseline)+'</div></div></section>'+
 '<p class="muted small">'+E(w.details)+'</p><div class="hero-actions"><button type="button" class="btn secondary n2-demo-close">'+E(w.end)+'</button></div></div>';
 function draw(message){
   modal.innerHTML=frame();
   modal.querySelectorAll('.n2-demo-close').forEach(b=>b.onclick=()=>modal.remove());
   modal.querySelector('.n2-demo-reset').onclick=()=>{result='pending';draw()};
   modal.querySelector('.n2-shared-demo-form').onsubmit=e=>{
     e.preventDefault();
     const choice=modal.querySelector('.n2-shared-demo-status').value,
       reason=modal.querySelector('.n2-shared-demo-reason').value.trim();
     if(choice==='shared'&&reason.length<8){
       modal.querySelector('.n2-demo-status').textContent=w.invalid;return;
     }
     result=choice;draw(w.saved);
   };
   if(message)modal.querySelector('.n2-demo-status').textContent=message;
 }
...
function renderHomeFull2(){
 ...
 v.innerHTML='<div class="hero"><div><div class="kicker">Kingdom 1044 · '+E(S.a)+'</div><h1>Dein NAP-Lagebild auf einen Blick.</h1><p>Eigene Maßnahmen, NAP-weite Hinweise und relevante Allianzwechsel – sauber nach Zuständigkeit getrennt.</p></div><div class="hero-actions"><button class="btn primary" data-go="add">＋ Verstoß eintragen</button><button class="btn secondary" data-go="players">Spielerakten</button>'+(S.a==='NRW'?'<button type="button" class="btn secondary n2-demo-open">🧪 '+E(sharedDemoWords2().cta)+'</button>':'')+'</div></div>'+
