...
async function renderCrownLive(){
 ...
 try{
  ...
  v.innerHTML='<div class="hero"><div><div class="kicker">CROWN · LIVE</div><h1>Minister-Ausschlüsse klar priorisiert.</h1><p>'+E(d?.privacy||'')+'</p></div><div class="hero-actions"><span class="pill gold">Königsallianz · '+E(k.alliance_code||'–')+'</span></div></div>'+
   '<div class="live-stat-grid"><div class="live-stat"><b>'+rows.length+'</b><small>aktive Minister-Sperren</small></div><div class="live-stat"><b>'+recent.length+'</b><small>beendet · 24h</small></div><div class="live-stat"><b>'+E(k.name||'–')+'</b><small>König</small></div><div class="live-stat"><b>'+E(D(k.refreshed_at))+'</b><small>aktualisiert</small></div></div>'+
   (!d?.has_access?'<div class="live-empty-state">Dieser Reiter ist nur für die aktuell gekrönte NAP-Allianz freigeschaltet.</div>':
...
let activityType2='all',activityAlliance2='';
...
async function renderActivityLive(){
 ...
 try{const d=await rpc('get_activity_log',{p_limit:250,p_offset:0,p_alliance:activityAlliance2||null,p_action:null});S.activity=Array.isArray(d)?d:[];paintActivity2()}catch(err){v.innerHTML+='<div class="live-empty-state">'+E(err.message||String(err))+'</div>'}
}
function paintActivity2(){
 const v=document.getElementById('view-activity');if(!v)return,types=[['all','Alle'],['uploads','Uploads'],['players','Spieler'],['events','Events'],['violations','Verstöße'],['deleted','Gelöscht']],rows=(S.activity||[]).filter(a=>activityType2==='all'||activityCategory2(a)===activityType2);
 ...
 '<div class="toolbar"><div class="toolbar-left">'+types.map(x=>'<button class="filter-chip '+(activityType2===x[0]?'active':'')+'" data-acttype="'+x[0]+'">'+x[1]+'</button>').join('')+'</div><div class="toolbar-right"><select id="liveActivityAlliance" class="compact-select"><option value="">Alle Allianzen</option>'+['NRW','THM','NWO','CWR','PxR','NwO'].map(a=>'<option '+(activityAlliance2===a?'selected':'')+'>'+a+'</option>').join('')+'</select></div></div>'+
 ...
 v.querySelectorAll('[data-acttype]').forEach(b=>b.onclick=()=>{activityType2=b.dataset.acttype;paintActivity2()});document.getElementById('liveActivityAlliance').onchange=e=>{activityAlliance2=e.target.value;renderActivityLive()};document.getElementById('liveActivityRefresh').onclick=renderActivityLive;
}
...
function renderPlayers2(){
 ...
 g.innerHTML=S.p.map(P=>{
  ...
  return '<div class="player-card" data-p="'+E(name)+'" data-has-entry="'+(V.length||ss(name).length?'1':'0')+'" data-attendance="'+(att?'1':'0')+'" data-search="'+E((name+' '+(P.game_id||'')).toLowerCase())+'">'+
   '<div class="player-card-top"><div class="player-meta">'+avatarHtml(P,'player-avatar')+'<div><div class="player-name">'+E(name)+'</div><div class="player-id">'+E(P.game_id||'–')+'</div></div></div><span class="pill">'+E(S.a)+'</span></div>'+
...
function profileActionCard2(s){
 const running=s.started_at&&!s.completed&&(!s.end_at||new Date(s.end_at)>new Date());
 return '<article class="card"><div class="card-head"><div><div class="card-title">Stufe '+E(s.level)+' · '+E(Number(s.level)===2?'R1':Number(s.level)===3?'24h NAP OUT':Number(s.level)===4?'Extended':'Kontakt')+'</div><div class="card-sub">Erstellt '+E(D(s.created_at))+'</div></div><span class="pill '+(s.completed?'green':running?'red':'gold')+'">'+(s.completed?'erledigt':'offen')+'</span></div><div class="card-body"><div class="action-date-grid"><div><span>Start</span><b>'+E(D(s.started_at))+'</b></div><div><span>Ende</span><b>'+E(D(s.end_at))+'</b></div><div><span>Restzeit</span><b>'+(s.end_at?E(dur(new Date(s.end_at)-Date.now())):'–')+'</b></div><div><span>Status</span><b>'+(s.completed?'erledigt':'offen')+'</b></div></div></div></article>';
}
async function openProfile2(name){
 ...
 view.innerHTML='<button class="btn small" data-go="players">← Spielerakten</button>'+
 '<div class="profile-head" style="margin-top:12px"><div class="profile-main">'+avatarHtml(P,'profile-avatar')+'<div><div class="kicker">SPIELERPROFIL</div><div class="profile-name">'+E(name)+'</div><div class="muted small">Player ID '+E(P.game_id||'–')+' · <span class="pill">'+E(S.a)+'</span> · '+E((P.languages||[]).join(' / ')||'–')+'</div></div></div></div>'+
 profileStage2(l)+
 '<div class="profile-tabs"><button class="profile-tab active" data-live-profiletab="overview">Übersicht</button><button class="profile-tab" data-live-profiletab="violations">Verstöße</button><button class="profile-tab" data-live-profiletab="actions">Maßnahmen</button><button class="profile-tab" data-live-profiletab="performance">Performance</button><button class="profile-tab" data-live-profiletab="comments">Kommentare</button><button class="profile-tab" data-live-profiletab="history">Historie</button></div><div id="liveProfileBody"></div>';
...
async function paintProfileTab2(name,tab){
 ...
 if(tab==='actions'){body.innerHTML='<div class="live-list">'+(X.length?X.map(profileActionCard2).join(''):'<div class="live-empty-state">Keine Maßnahmen.</div>')+'</div>';return}
 ...
 const events=[...V.map(v=>({d:v.occurred_at||v.created_at,t:v.event_name,s:v.phase_name||'',k:'Verstoß'})),...X.map(x=>({d:x.created_at,t:'Stufe '+x.level,s:x.completed?'erledigt':'offen',k:'Maßnahme'}))].filter(x=>x.d).sort((a,b)=>new Date(b.d)-new Date(a.d));body.innerHTML='<div class="live-list">'+(events.length?events.map(x=>'<div class="live-row"><div><b>'+E(x.k)+' · '+E(x.t||'–')+'</b><small>'+E(x.s||'')+'</small></div><time class="muted tiny">'+E(D(x.d))+'</time></div>').join(''):'<div class="live-empty-state">Keine Historie.</div>')+'</div>';
}
...
function renderHomeFull2(){
 const v=document.getElementById('view-home');if(!v)return,A=actions(),recent=S.v.slice(0,8);
 ...
 '<div class="grid stat-grid home-kpis"><div class="stat-card"><div class="stat-top"><span>Eigene offene Maßnahmen</span></div><div class="stat-value">'+A.length+'</div><div class="stat-sub">'+E(S.a)+'</div></div><div class="stat-card"><div class="stat-top"><span>NAP-weit überfällig</span></div><div class="stat-value">'+S.o.length+'</div><div class="stat-sub">andere NAP-Allianzen</div></div><div class="stat-card"><div class="stat-top"><span>Aktive NAP OUT</span></div><div class="stat-value">'+S.e.length+'</div><div class="stat-sub">NAP-weit</div></div><div class="stat-card"><div class="stat-top"><span>Allianzwechsel</span></div><div class="stat-value">'+S.t.length+'</div><div class="stat-sub">nur '+E(S.a)+' betreffend</div></div></div>'+
 '<div class="home-main-grid"><div class="stack"><section class="card"><div class="card-head"><div><div class="card-title">Mein Handlungsbedarf</div><div class="card-sub">direkt auf Home erledigen</div></div><span class="pill gold">'+A.length+' offen</span></div><div class="card-body">'+(A.length?A.map(a=>{const P=p(a.name),lab=a.l===1?'Stufe 1 · Kontakt':a.l===2?'Stufe 2 · R1':a.l===3?'Stufe 3 · 24h NAP OUT':'Stufe 4 · Extended',b=a.l===1?'<button class="btn small primary n2act" data-k="contact" data-id="'+E(a.v.id)+'">'+E(t('contact'))+'</button>':a.l===2?'<button class="btn small primary n2act" data-k="r1" data-id="'+E(a.s.id)+'">'+E(t('r1'))+'</button>':a.l===3?'<button class="btn small primary n2act" data-k="nap" data-id="'+E(a.s.id)+'">'+E(t('nap'))+'</button>':'';return '<div class="action-item"><div><div class="player-line">'+avatarHtml(P,'player-avatar')+'<div><div class="player-name">'+E(a.name)+'</div><div class="player-id">ID '+E(P.game_id||'–')+' · '+E(S.a)+'</div></div></div></div><div class="action-right"><span class="pill gold">'+E(lab)+'</span><span class="deadline">'+E(dl(a))+'</span><div class="home-action-buttons">'+b+'<button class="mini-link n2open" data-p="'+E(a.name)+'">Öffnen</button></div></div></div>'}).join(''):'<div class="live-empty-state">✓ Aktuell kein eigener Handlungsbedarf.</div>')+'</div></section>'+
 '<section class="card"><div class="card-head"><div><div class="card-title">Letzte eigene Verfehlungen</div><div class="card-sub">private Details bleiben bei '+E(S.a)+'</div></div></div><div class="card-body live-list">'+(recent.length?recent.map(x=>'<div class="live-row"><div><b>'+E(x.player_name)+'</b><small>'+E(x.event_name||'')+' · '+E(x.phase_name||'')+' · '+E(D(x.occurred_at))+'</small></div><strong>'+N(x.score)+'</strong></div>').join(''):'<div class="live-empty-state">Keine Verfehlungen.</div>')+'</div></section></div>'+
 '<div class="stack"><section class="card"><div class="card-head"><div><div class="card-title">🔔 NAP-Benachrichtigungen</div><div class="card-sub">überfällige Maßnahmen anderer NAP-Allianzen</div></div><span class="pill red">'+S.o.length+'</span></div><div class="card-body live-list">'+(S.o.length?S.o.slice(0,6).map(x=>'<div class="live-row"><div><b>'+E(x.alliance_code)+' · '+E(x.player_name||'–')+'</b><small>Stufe '+E(x.level)+'</small></div><span class="pill red">'+E(dur(Number(x.overdue_seconds||0)*1000))+'</span></div>').join(''):'<div class="live-empty-state">Keine überfälligen Maßnahmen.</div>')+'</div></section>'+
 '<section class="card"><div class="card-head"><div><div class="card-title">Allianzwechsel</div><div class="card-sub">nur Quelle oder Ziel '+E(S.a)+'</div></div><span class="pill">'+S.t.length+'</span></div><div class="card-body live-list">'+(S.t.length?S.t.map(x=>'<div class="live-row"><div><b>'+E(x.player_name||'–')+'</b><small>'+E(x.from_alliance||'POOL')+' → '+E(x.to_alliance||'POOL')+'</small></div><span class="pill '+(x.to_alliance===S.a?'green':'blue')+'">'+E(D(x.detected_since))+'</span></div>').join(''):'<div class="live-empty-state">Keine relevanten Wechsel.</div>')+'</div></section><section class="card" id="liveHomePerformance"><div class="card-body"><div class="live-empty-state">Performance wird geladen …</div></div></section></div></div>';
 v.querySelectorAll('.n2act').forEach(b=>b.onclick=()=>doAct(b.dataset.k,b.dataset.id));v.querySelectorAll('.n2open').forEach(b=>b.onclick=()=>openProfile2(b.dataset.p));renderHomePerformance2();renderNotifications2();
}
