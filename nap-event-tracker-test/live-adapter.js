...
function transferRow2(x){
 ...
 return '<div class="live-row live-transfer-row"><div><b>'+E(x.player_name||'–')+'</b><small>'+E(x.from_alliance||'POOL')+' → '+E(x.to_alliance||'POOL')+' · '+E(D(x.detected_since))+'</small>'+(buttons?'<div class="hero-actions" style="margin-top:7px">'+buttons+'</div>':'')+'</div><span class="pill '+(ownTarget?'green':'blue')+'">'+E(ownTarget?'Eingang':'Ausgang')+'</span></div>';
}
