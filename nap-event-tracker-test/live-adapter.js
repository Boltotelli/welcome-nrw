...
function addLiveCss(){
 ...
 '@media(max-width:900px){.live-panel-grid{grid-template-columns:1fr}.live-stat-grid,.performance-mob-card .live-stat-grid,.performance-kvk-card .live-stat-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.live-form-row{grid-template-columns:1fr}.live-import-frame{min-height:580px}}';
...
async function renderPerformanceLive(){
 ...
 try{
  ...
  v.innerHTML='<div class="hero"><div><div class="kicker">PERFORMANCE · LIVE</div><h1>Performance ohne Dashboard-Überladung.</h1><p>'+E(S.a)+' · höchste Werte pro Spieler</p></div><div class="hero-actions"><button class="btn primary" id="livePerfScreen">ScreenRecording importieren</button><button class="btn secondary" id="livePerfManual">Manuell ergänzen / korrigieren</button></div></div>'+
   '<div class="live-panel-grid"><section class="card performance-summary-card performance-mob-card"><div class="card-head"><div><div class="card-title">Alliance Mobilization</div><div class="card-sub">'+E(m?.event?.label||'')+'</div></div></div><div class="card-body">'+
