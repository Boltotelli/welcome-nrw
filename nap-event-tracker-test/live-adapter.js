...
function profileActionCard2(s){
 const running=s.started_at&&!s.completed&&(!s.end_at||new Date(s.end_at)>new Date()),r1Missing=Number(s.level)===2&&s.completed&&(!s.started_at||!s.end_at);
 return '<article class="card" data-profile-sanction="'+E(s.id)+'"><div class="card-head"><div><div class="card-title">Stufe '+E(s.level)+' · '+E(Number(s.level)===2?'R1':Number(s.level)===3?'24h NAP OUT':Number(s.level)===4?'Extended':'Kontakt')+'</div><div class="card-sub">Erstellt '+E(D(s.created_at))+'</div></div><span class="pill '+(r1Missing?'gold':s.completed?'green':running?'red':'gold')+'">'+E(r1Missing?'Timer fehlt':s.completed?'erledigt':'offen')+'</span></div><div class="card-body">'+(r1Missing?'<div class="notice warn" style="margin-bottom:10px">⚠ R1 bestätigt – individuelle Endzeit noch setzen.</div><div class="live-form-row"><label>Ende<input class="profile-r1-end" type="datetime-local" value="'+E(toLocalInput2(s.end_at))+'"></label><div style="display:flex;align-items:end"><button class="btn secondary profile-r1-save" data-id="'+E(s.id)+'">Timer setzen</button></div></div>':'')+'<div class="action-date-grid"><div><span>Start</span><b>'+E(D(s.started_at))+'</b></div><div><span>Ende</span><b>'+E(D(s.end_at))+'</b></div><div><span>Restzeit</span><b>'+(s.end_at?E(dur(new Date(s.end_at)-Date.now())):'–')+'</b></div><div><span>Status</span><b>'+E(r1Missing?'Timer fehlt':s.completed?'erledigt':'offen')+'</b></div></div></div></article>';
}
