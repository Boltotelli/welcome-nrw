...
function addLiveCss(){
 ...
 '.live-stat-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin-bottom:13px}.live-stat{padding:12px;border:1px solid var(--line);border-radius:13px;background:var(--panel);min-width:0}.live-stat b{font-size:20px;display:block;max-width:100%;font-variant-numeric:tabular-nums;line-height:1.12}.live-stat small{color:var(--muted);font-size:8px;text-transform:uppercase;letter-spacing:.06em}.performance-summary-card{container-type:inline-size}.performance-summary-card .live-stat-grid{grid-template-columns:repeat(4,minmax(0,1fr))}@container (max-width:700px){.performance-summary-card .live-stat-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@container (max-width:380px){.performance-summary-card .live-stat-grid{grid-template-columns:1fr}}'+
...
async function renderPerformanceLive(){
 ...
 try{
  ...
  v.innerHTML='<div class="hero"><div><div class="kicker">PERFORMANCE · LIVE</div><h1>Performance ohne Dashboard-Überladung.</h1><p>'+E(S.a)+' · höchste Werte pro Spieler</p></div><div class="hero-actions"><button class="btn primary" id="livePerfScreen">ScreenRecording importieren</button><button class="btn secondary" id="livePerfManual">Manuell ergänzen / korrigieren</button></div></div>'+
   '<div class="live-panel-grid"><section class="card performance-summary-card"><div class="card-head"><div><div class="card-title">Alliance Mobilization</div><div class="card-sub">'+E(m?.event?.label||'')+'</div></div></div><div class="card-body">'+
   ...
   '<section class="card performance-summary-card"><div class="card-head"><div><div class="card-title">KvK Prep · Top 200</div><div class="card-sub">'+E(k?.event?.label||'')+'</div></div></div><div class="card-body">'+
