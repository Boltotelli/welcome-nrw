 ...
 try{
  ...
  for(let i=0;i<times.length;i++){
   ...
   for(const row of rows){
    ...
    if(!p){const k=norm(row.name)+'|'+row.score;if(!unmatched.has(k))unmatched.set(k,{...row,time:sec});continue}
...
function showReview(r){
 ...
 (r.unmatched.length?'<details class="nocr-unmatched"><summary>'+esc(tr('unmatched'))+' ('+r.unmatched.length+')</summary>'+r.unmatched.map(x=>'<p>'+esc(x.raw)+'</p>').join('')+'</details>':'');
...
function mount(root,kind,allowed=[]){
 if(run?.busy)return;
 run={root,kind,allowed,hits:[],unmatched:[],occurrences:[],busy:false,preview:null,fileHash:null};
 shell(root,kind);
