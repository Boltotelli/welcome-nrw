...
function p2PlayerCard(p){const name=p2PlayerName(p),vs=p2Violations(name),active=p2ActiveViolations(name),level=p2CurrentLevel(name),status=p2CardStatus(name),last=p2LatestDate(name);return '<button class="p2-player-card" type="button" data-p2-player="'+v2esc(name)+'"><div class="p2-card-head"><div class="p2-identity"><div class="p2-avatar">'+v2esc(p2Initial(name))+'</div><div><strong>'+v2esc(name)+'</strong><small>'+v2esc(p.game_id||'–')+'</small></div></div><span class="p2-alliance-badge">'+v2esc(p.alliance_code||state.alliance)+'</span></div><div class="p2-metrics"><div><b>'+vs.length+'</b><small>'+v2esc(p2t('violations'))+'</small></div><div><b>'+level+'</b><small>'+v2esc(p2t('stage'))+'</small></div><div><b>'+v2esc(status.value)+'</b><small>'+v2esc(status.label)+'</small></div></div><div class="p2-card-foot"><div>'+p2AttendanceBadge(name)+(active.length?'<span class="p2-action-dot">'+v2esc(p2t('actionNeeded'))+'</span>':'')+'</div><small>'+(last?v2esc(v2date(last))+' · '+v2esc(p2t('lastEntry')):'–')+'</small></div></button>'}
...
function p2SanctionHtml(s){const level=Number(s.level),running=!!s.started_at&&!s.completed&&(!s.end_at||new Date(s.end_at)>new Date()),remainingText=s.end_at?v2duration(Math.max(0,(new Date(s.end_at)-Date.now())/1000)):'–';return '<article class="p2-record-card"><div class="p2-record-head"><div><b>'+v2esc(p2t('stage'))+' '+level+' · '+v2esc(v2LevelLabel(level))+'</b><small>'+v2esc(p2t('created'))+' '+v2esc(v2date(s.created_at))+'</small></div><span class="v2-pill '+(s.completed?'green':running?'red':'gold')+'">'+v2esc(s.completed?p2t('completed'):p2t('pending'))+'</span></div><div class="p2-record-meta"><span><small>'+v2esc(p2t('implemented'))+'</small><b>'+v2esc(s.started_at?v2date(s.started_at):'–')+'</b></span><span><small>'+v2esc(p2t('ends'))+'</small><b>'+v2esc(s.end_at?v2date(s.end_at):'–')+'</b></span><span><small>'+v2esc(p2t('remaining'))+'</small><b>'+v2esc(s.end_at?remainingText:'–')+'</b></span></div></article>'}
