...
function p2StageCompleted(name,n){
 const s=p2Sanctions(name).find(x=>Number(x.level)===n);
 if(!s)return false;
 if(n===1){
  const v=p2Violations(name).find(x=>String(x.id)===String(s.violation_id));
  return !!(v?.contacted||s.completed);
 }
 if(n===2)return !!(s.completed&&s.started_at&&s.end_at);
 if(n===3)return !!(s.completed&&s.started_at&&s.end_at&&new Date(s.end_at).getTime()<=Date.now());
 if(n===4)return !!(s.started_at&&s.end_at&&new Date(s.end_at).getTime()<=Date.now());
 return false;
}
function p2StageTimeline(name,level){
 const labs=[p2t('stage1'),p2t('stage2'),p2t('stage3'),p2t('stage4')];
 return '<div class="p2-stage-wrap"><div class="p2-stage-line">'+labs.map((lab,i)=>{
  const n=i+1,current=n===level,completed=p2StageCompleted(name,n),done=n<level||completed;
  const status=current?(completed?'✓ '+p2t('stageDone')+' · '+p2t('current'):p2t('current')):(n<level?'✓':'');
  return '<div class="p2-stage-node '+(done?'done ':'')+(current?'current ':'')+(current&&completed?'current-complete':'')+'"><span>'+n+'</span><small>'+v2esc(lab)+(status?'<em>'+v2esc(status)+'</em>':'')+'</small></div>';
 }).join('')+'</div></div>';
}
